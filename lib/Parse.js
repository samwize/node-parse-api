    ...
    find: function (className, query, callback) {
        ...
        if ( queryType === 'string' ) {
            ...
            if ( query.hasOwnProperty('where') ) {
              var encodedComponent = encodeURIComponent(JSON.stringify(query.where))
              if (encodedComponent != null && encodedComponent != "null") {
                console.log(encodedComponent)
                url += '?where=' + encodedComponent;
              }
              delete query.where;
            }
...
function parseRequest(method, path, data, callback, contentType, sessionToken) {
    ...
    if(this._options.master_key){
        ...
        headers['X-Parse-MASTER-Key'] = this._options.master_key;
        if ( sessionToken ) {
